...
下拉展示面板（含顶栏余额显示）。对角线两侧是同一套布局的两套自绘配色，分别对应系统的深色与浅色；图中的服务商数据来自本地演示快照。
...
- 顶栏里快照中每个服务商各占一项（与直连服务商同等对待），并按紧急度排序：剩余百分比最紧的在前，其次是额度/今日费用，失败的行排在最后（失败仍会计入右上角 ⚠ 角标）。“单显”模式最多并列 2 项，其余折叠成 `+N`（再多就会和顶栏中间的时钟重叠）；“轮播”模式逐项循环，不折叠。
- 面板底部显示本扩展的上次刷新时间与 CodexBar 快照的生成时间。
- 快照按“任何键都可能缺失”来解析：只有确实存在的数字才会展示，缺数字的窗口不会被当成 0%，上游新增字段也不影响旧版本读取。
- 上游标记为 `idle`（该模型族没有用量）的窗口不展示；未知类型的窗口统一显示为“配额窗口”，能拿到窗口时长时按时长标注，不会把上游内部 id 直接暴露到面板。
- 多账号集成（如 claude-swap）可能只在账号行上给出用量：顶层窗口为空时取活动账号的数据；整个账号采集失败（`accountsError`）只作为提示行显示，不会把卡片标记为查询失败。
- 某一行数据的 `updatedAt` 超过快照给出的陈旧阈值（`staleAfterSeconds`）时，卡片会标注“N 分/小时前更新”，提示这是旧数据而非实时值。
- 边界：桥接层展示的是 CodexBar 的**配额窗口 / 额度**视图。按余额计费的服务商（DeepSeek、OpenRouter 等）在快照里往往只有一个 100% 的占位窗口而没有金额，要看余额请使用对应的直连服务商。
